const { obtenerTeachers, crearTeacher, obtenerTeacherPorEmail, actualizarTeacher, deleteTeacher } = require('../services/teacher_service');
...
const deleteadoTeacher=async(req,res,next) => {
    try {
        const teacher=await deleteTeacher(req.params.id);
        if (!teacher) {
            res.status(404).json({"Error": "Teacher no encontrado"})
        }
...
module.exports = { getTeacher, createTeacher,updateTeacher,deleteadoTeacher };